    ...
    <tbody>
        <tr>
            ...
            <td class="reply">
                ...
                {if $enable_remove_post}<span><a href="{$DIR_PATH}/remove_post.php?post={$post.id}"><img src="{$DIR_PATH}/css/{$STYLESHEET}/delete.png" alt="[Удалить]" title="Удалить сообщение" border="0" width="16" height="16"/></a></span>{else}<!-- Link to remove post disabled -->{/if}
                ...
                {if $enable_extrabtns}<span class="extrabtns">
                    {if $post.with_attachments}<a href="{$DIR_PATH}/remove_upload.php?post={$post.id}"><img src="{$DIR_PATH}/css/{$STYLESHEET}/delfile.png" alt="[Удалить файл]" title="Удалить файл" border="0" width="16" height="16"/></a>{else}<!-- There is no attachments -->{/if}
...
{$post.text}
                    {if $post.text_cutted == 1}<div class="abbrev">Нажмите <a href="{$DIR_PATH}/{$post.board.name}/{$post.thread.original_post}#{$post.number}">Ответ</a> для просмотра сообщения целиком.</div>{else}<!-- Text not cutted -->{/if}
