 ...
 *********************************}
{*
Code of post on moderators main page.
...
<td colspan="3" class="reply">
    ...
    {if $post.tripcode != null}<span class="postertrip">!{$post.tripcode}</span>{else}<!-- There is no tripcode -->{/if}

    {if $author_admin}<span class="admin">❀❀&nbsp;Админ&nbsp;❀❀</span>{else}<!-- Author is not admin -->{/if}
